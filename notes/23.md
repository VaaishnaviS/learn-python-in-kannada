...
## 💡 Real-World Example (Kannada Context)

```python
with open("karnataka_districts.txt", "w") as file:
    file.write("Bengaluru\nMysuru\nDharwad")

with open("karnataka_districts.txt", "r") as file:
    print("Namma Karnataka Districts:")
    for line in file:
        print("-", line.strip())
```
